...
The `EAGGA` algorithm is a model-agnostic framework designed to enhance the predictive performance and interpretability of supervised machine learning models for tabular data.
...
Efficient optimization is achieved by expanding the search space of the learning algorithm through the inclusion of feature selection, interaction, and monotonicity constraints in the hyperparameter search.
...
To facilitate this process, `EAGGA` introduces a novel evolutionary algorithm specifically designed to operate effectively within the augmented search space.
...
Finally, the `eagga` package builds upon the `mlr3` ecosystem.
If you are not familiar with it, we highly recommend to make your self familiar with it, by for example reading the following book: https://mlr3book.mlr-org.com/
...
If your use case includes categorical features, please open an issue so that we are aware of it and can work on implementing proper support with higher priority.
...
Moreover, the function to apply via `r ref("mlr3pipelines::PipeOpColApply", "PipeOpColApply")` must be set to `function(x) - x` (which is responsible for changing the sign of features after having detected it via using a monotonicity detector as described above).

Users can customize this pipeline but must adhere to the basic structure (`colapply --> select --> sortfeatures --> [custom user block] --> learner`).
...
Note that in general it is better to set the upper bound of `nrounds` higher (e.g., around `log(5000)` and the default to `log(500)` but we use less boosting rounds to speed up the following example.

Additionally, we must specify that we also tune over the feature `selector` and `interaction_constraints` and `monotone_constraints` (we specify these as `r ref("paradox::ParamUty", "ParamUty")` hyperparameters because they are not tunable in a standard way.
...
As `EAGGA` jointly optimizes for performance and interpretability, we also need to pass the interpretability measures (`NF`, `NI` and `NNM` in the paper).

These measures are implemented via so-called proxy measures (proxy because they actually do not work like standard measures and operate on a `r ref("mlr3::Prediction", "Prediction")` object but rather are always hard coded to a value of zero but updated and computed within the optimization process.

For details, see `r ?MeasureSelectedFeaturesProxy`, `r ?MeasureSelectedInteractionsProxy` and `r ? MeasureSelectedNonMonotoneProxy`.
...
As we have introduced all building blocks, we can now use `EAGGA` to jointly tune for performance and interpretability.
...
By setting `terminator = trm("evals", n_evals = 30)` we specified that termination of the tuning process should occur after having evaluated 30 configurations.
...
Furthermore we can specify `seed_calculate_proxy_measures` to seed the model fitting process of configuration tried during optimization on the task to be able to fully reproduce models found during optimization.
...
Note: As this process of swapping the sign of features is performed during optimization, you cannot simply use the learner you pass to the instance afterwards to train your learner with a given hyperparameter configuration and group structure.
Instead, you should use the learner passed to the instance (which is updated in place): `instance$objective$learner`.
To use a hyperparameter configuration and group structure together with this learner, you can then do the following.
Assume we want to use the eight configuration of the Pareto set:
...
Once we have decided on a model (i.e., non-dominated hyperparameter configuration and group structure) we prefer, we can "reconstruct" it.
...
We know that feature `"x3"` can be used by the model and is constrained to have a monotone increasing effect on the target.
