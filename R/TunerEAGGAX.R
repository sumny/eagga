...
#'
#' @export
TunerEAGGAX = R6Class("TunerEAGGAX",
  ...
  public = list(
    ...
    initialize = function() {
      param_set = ps(
        ...
        random = p_lgl(default = FALSE, tags = "required")
      )
      param_set$values = list(select_id = "select.selector", interaction_id = "classif.xgboost.interaction_constraints", monotone_id = "classif.xgboost.monotone_constraints", crossover = TRUE, mutation = TRUE, detectors = TRUE, random = TRUE)
      super$initialize(
        param_set = param_set,
        ...
        man = "eagga::mlr_tuners_eagga_ablation"
      )
    }
  ),
  ...
  private = list(
    .optimize = function(inst) {
      ...
      if (self$param_set$values$detectors) {
        ...
        scores = as.data.table(filter$calculate(task))  # filter scores are used to weight probabilities of inclusion in GroupStructureX
      ...
      sIm = map_dtr(seq_len(mu - 1L), function(i) {  # sIm space
        groupstructure = GroupStructureX$new(task, n_selected = n_selected[i], scores = scores, interaction_detector = interaction_detector, unconstrained_weight_table = unconstrained_weight_table, use_detectors = self$param_set$values$detectors)
      ...
      groupstructure_unconstrained = GroupStructureX$new(task, n_selected = length(task$feature_names), scores = scores, interaction_detector = interaction_detector, unconstrained_weight_table = unconstrained_weight_table, unconstrained = TRUE, use_detectors = self$param_set$values$detectors)
      ...
      repeat {  # iterate until we have an exception from eval_batch
        ...
        children = map_dtr(seq_len(ceiling(lambda / 2)), function(i) {
          if (self$param_set$values$random) {  # children are generated at "random" like during initialization
            random_children = map_dtr(seq_len(2L), function(child) {
              x_child = as.data.table(param_space$default)
              for (param_id in param_ids) {
                x_child[1L, ][[param_id]] = mutate(x_child[1L, ][[param_id]], param = param_space$params[[param_id]], p = 1)  # mutate the defaults with probability 1
              }

              n_selected_child = sample_from_truncated_geom(n_selected_prob, lower = 1L, upper = length(task$feature_names))
              groupstructure_child = GroupStructureX$new(task, n_selected = n_selected_child, scores = scores, interaction_detector = interaction_detector, unconstrained_weight_table = unconstrained_weight_table, use_detectors = self$param_set$values$detectors)
              sIm_child = data.table(groupstructure = list(groupstructure_child),
                                     s = list(groupstructure_child$create_selector()),
                                     I = list(groupstructure_child$create_interaction_constraints()),
                                     m = list(groupstructure_child$create_monotonicity_constraints()))
              colnames(sIm_child) = c("groupstructure", select_id, interaction_id, monotone_id)
